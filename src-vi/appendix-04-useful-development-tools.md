...
Cài đặt Rust bao gồm rustfmt theo mặc định, vì vậy bạn nên đã có sẵn các chương
trình `rustfmt` và `cargo-fmt` trên hệ thống của mình. Hai lệnh này tương tự như
`rustc` và `cargo` ở chỗ `rustfmt` cho phép kiểm soát chi tiết hơn và
...
nên thay đổi phong cách mã, không phải ngữ nghĩa mã.

Lệnh này cung cấp cho bạn `rustfmt` và `cargo-fmt`, tương tự như cách Rust cung
cấp cho bạn cả `rustc` và `cargo`. Để định dạng bất kỳ dự án Cargo nào, hãy nhập
như sau:
...
muốn. Có thể bạn đã thấy các cảnh báo của trình biên dịch trước đây. Ví dụ, hãy
xem xét mã này:
...
phiên bản Rust khác nhau. Các phiên bản được đề cập trong [Phụ lục E][editions].
...
thay vì. Sau đó bạn sẽ thay đổi mã của mình để sử dụng hằng số `PI`. Mã sau đây
không gây ra bất kỳ lỗi hoặc cảnh báo nào từ Clippy:
...
Để hỗ trợ tích hợp IDE, cộng đồng Rust khuyên dùng
[`rust-analyzer`][rust-analyzer]<!-- ignore -->. Công cụ này là một tập hợp các
tiện ích tập trung vào trình biên dịch nói chuyện theo [Giao thức Máy chủ Ngôn
ngữ][lsp]<!-- ignore -->, là một đặc điểm kỹ thuật để IDE và ngôn ngữ lập trình
giao tiếp với nhau. Các client khác nhau có thể sử dụng `rust-analyzer`, chẳng
hạn như [plug-in Rust analyzer cho Visual Studio Code][vscode].

[lsp]: http://langserver.org/
[vscode]:
  https://marketplace.visualstudio.com/items?itemName=rust-lang.rust-analyzer

Truy cập [trang chủ][rust-analyzer]<!-- ignore --> của dự án `rust-analyzer` để
biết hướng dẫn cài đặt, sau đó cài đặt hỗ trợ máy chủ ngôn ngữ trong IDE cụ thể
của bạn. IDE của bạn sẽ có các khả năng như tự động hoàn thành, nhảy đến định
nghĩa và hiển thị lỗi trực tiếp.
