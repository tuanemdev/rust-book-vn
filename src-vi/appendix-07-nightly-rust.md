...
cả sự phát triển được thực hiện trên nhánh `master` của kho lưu trữ Rust. Các
phiên bản phát hành tuân theo mô hình đoàn tàu phát hành phần mềm, đã được sử
dụng bởi Cisco IOS và các dự án phần mềm khác. Có ba _kênh phát hành_ cho Rust:
...
nhánh `master`. Mỗi đêm, một phiên bản nightly mới của Rust được tạo ra. Mỗi
ngày là một ngày phát hành, và các phiên bản này được tạo ra bởi cơ sở hạ tầng
phát hành của chúng tôi một cách tự động. Vì vậy, theo thời gian, các phiên bản
của chúng tôi trông như thế này, mỗi đêm một lần:
...
trữ Rust tách ra từ nhánh `master` được sử dụng bởi nightly. Bây giờ, có hai
phiên bản:
...
áp dụng cho `master`, để nightly được sửa, và sau đó bản sửa lỗi được chuyển lại
cho nhánh `beta`, và một phiên bản beta mới được tạo ra:
...
tích cực, nó sẽ được đưa vào `master`, và do đó, trong nightly, nhưng đằng sau
...
`master` đằng sau một cờ tính năng, như chúng tôi đã thảo luận trong phần
