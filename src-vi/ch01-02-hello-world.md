...
> bạn, vì vậy nếu bạn thích sử dụng môi trường phát triển tích hợp (IDE) thay vì
> dòng lệnh, hãy tự nhiên sử dụng IDE yêu thích của bạn. Nhiều IDE hiện nay có
> một số mức độ hỗ trợ Rust; hãy kiểm tra tài liệu của IDE để biết chi tiết. Đội
> ngũ Rust đã tập trung vào việc hỗ trợ IDE thông qua `rust-analyzer`. Xem [Phụ
> lục D][devtools]<!-- ignore --> để biết thêm chi tiết.
...
biểu thức này đã kết thúc và biểu thức tiếp theo đã sẵn sàng để bắt đầu. Hầu hết
các dòng mã Rust kết thúc bằng dấu chấm phẩy.
