...
`rand` với chỉ định phiên bản ngữ nghĩa `0.10.1`. Cargo hiểu [Semantic
Versioning][semver]<!-- ignore --> (đôi khi được gọi là _SemVer_), là một tiêu
chuẩn để viết số phiên bản. Chỉ định `0.10.1` thực sự là viết tắt của `^0.10.1`,
có nghĩa là bất kỳ phiên bản nào ít nhất là 0.10.1 nhưng dưới 0.11.0.

Cargo coi các phiên bản này có API công khai tương thích với phiên bản 0.10.1, và
chỉ định này đảm bảo bạn sẽ nhận được bản phát hành vá lỗi mới nhất mà vẫn biên
dịch được với mã trong chương này. Bất kỳ phiên bản nào từ 0.11.0 trở lên không
...
rằng tuần tới phiên bản 0.10.2 của crate `rand` ra mắt, và phiên bản đó chứa một
...
bản 0.10.1 cho đến khi bạn nâng cấp rõ ràng, nhờ vào file _Cargo.lock_. Vì file
...
hơn 0.10.1 và nhỏ hơn 0.11.0. Nếu crate `rand` đã phát hành hai phiên bản mới
0.10.2 và 0.999.0, bạn sẽ thấy điều sau nếu bạn chạy `cargo update`:
...
Cargo bỏ qua phiên bản 0.999.0. Tại thời điểm này, bạn cũng sẽ nhận thấy một thay
đổi trong file _Cargo.lock_ ghi nhận rằng phiên bản của crate `rand` mà bạn đang
sử dụng bây giờ là 0.10.2. Để sử dụng phiên bản 0.999.0 của `rand` hoặc bất kỳ
phiên bản nào trong loạt 0.999._x_, bạn sẽ phải cập nhật file _Cargo.toml_ để
trông như thế này:
...
Đầu tiên, chúng ta thêm dòng `use rand::prelude::*;`. Module `prelude` chứa các
phần được sử dụng phổ biến nhất của crate `rand`, và `use` làm cho các item đó có
sẵn trong phạm vi chương trình của chúng ta.

Tiếp theo, chúng ta thêm hai dòng ở giữa. Trong dòng đầu tiên, chúng ta gọi hàm
`rand::rng` để lấy bộ tạo số ngẫu nhiên cụ thể mà chúng ta sẽ sử dụng: một bộ tạo
số ngẫu nhiên cục bộ cho luồng thực thi hiện tại và được khởi tạo bởi hệ điều
hành. Sau đó, chúng ta gọi phương thức `random_range` trên bộ tạo số ngẫu nhiên.
Phương thức này được định nghĩa bởi trait `RngExt` là một phần của module
`rand::prelude` mà chúng ta đã đưa vào phạm vi với câu lệnh `use
rand::prelude::*;`. Phương thức `random_range` lấy một biểu thức phạm vi làm đối
số và tạo ra một số ngẫu nhiên trong phạm vi đó. Loại biểu thức phạm vi mà chúng
ta đang sử dụng ở đây có dạng `start..=end` và bao gồm cả giới hạn dưới và giới
hạn trên, vì vậy chúng ta cần chỉ định `1..=100` để yêu cầu một số từ 1 đến 100.

> Lưu ý: Bạn sẽ không chỉ biết cần đưa gì vào phạm vi và các phương thức và
...
1 đến 100. Nếu bạn nhận được cảnh báo, chúng có thể được bỏ qua một cách an toàn.
Nếu bạn nhận được lỗi, vui lòng kiểm tra rằng bạn có `rand = "0.10.1"` trong
*Cargo.toml* của bạn vì các phiên bản `rand` trong tương lai có thể có API khác,
nhưng bất kỳ phiên bản nào trong loạt `0.10` đều sẽ hoạt động với mã trong chương
này.
