...
_receiver_ (bộ thu), vì vậy chúng ta đặt tên cho các biến của mình như vậy để
...
trong Chương 19. Hiện tại, hãy biết rằng sử dụng câu lệnh `let` theo cách này là
...
được tạo ra đang giao tiếp với thread chính, như được hiển thị trong Listing
...
Lỗi đồng thời của chúng ta đã gây ra một lỗi biên dịch. Hàm `send` lấy quyền sở
...
bằng cách gọi hàm `thread::sleep` với một giá trị `Duration` là một giây.
...
Khi chạy mã trong Listing 16-10, bạn sẽ thấy đầu ra sau với một khoảng dừng một
giây giữa mỗi dòng:
