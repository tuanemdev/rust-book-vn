...
muốn truy cập bằng cách yêu cầu có được _khóa_ của mutex. Khóa là một cấu trúc
...
Như bạn có thể nghi ngờ, `Mutex<T>` là một con trỏ thông minh. Chính xác hơn,
lệnh gọi đến `lock` _trả về_ một con trỏ thông minh được gọi là `MutexGuard`,
được bọc trong một `LockResult` mà chúng ta đã xử lý bằng cách gọi `unwrap`. Con
trỏ thông minh `MutexGuard` thực hiện `Deref` để trỏ đến dữ liệu bên trong của
chúng ta; con trỏ thông minh cũng có một triển khai `Drop` giải phóng khóa một
...
sở hữu của khóa `counter` vào nhiều thread. Hãy sửa lỗi trình biên dịch bằng
