...
`Arc<T>`, vốn triển khai `Send`, mã đã biên dịch thành công.

Bất kỳ kiểu nào được tạo thành hoàn toàn từ các kiểu `Send` cũng tự động được
đánh dấu là `Send`. Hầu hết tất cả các kiểu nguyên thủy đều là `Send`, ngoại trừ
con trỏ thô, thứ mà chúng ta sẽ thảo luận trong Chương 20.
...
chiếu có thể được gửi an toàn đến một thread khác. Tương tự như `Send`, các kiểu
nguyên thủy đều triển khai `Sync`, và các kiểu được tạo thành hoàn toàn từ các
