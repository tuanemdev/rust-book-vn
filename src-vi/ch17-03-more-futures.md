...
Nhớ lại từ phần ["Our First Async Program"][async-program]<!-- ignore -->
rằng tại mỗi điểm await, Rust cho phép runtime có cơ hội tạm dừng
tác vụ và chuyển sang một tác vụ khác nếu future đang được await chưa sẵn sàng. 
Điều ngược lại cũng đúng: Rust _chỉ_ tạm dừng các async block và trả quyền kiểm 
soát lại cho runtime tại một điểm await. Mọi thứ giữa các điểm await là đồng bộ.
...
Hãy mô phỏng một hoạt động chạy dài để minh họa vấn đề đói, sau đó khám phá 
cách giải quyết nó. Listing 17-14 giới thiệu một hàm `slow`.
...
<Listing number="17-15" caption="Gọi `slow` để mô phỏng chạy các hoạt động chậm" file-name="src/main.rs">
...
bàn giao quyền kiểm soát thông qua các điểm await. Mỗi future do đó cũng có trách 
nhiệm tránh chặn quá lâu. Trong một số hệ điều hành nhúng dựa trên Rust, đây là 
_loại duy nhất_ của đa nhiệm!
...
ngắn. Nếu `future_to_try` hoàn thành trước, `select` sẽ trả về `Left` với đầu 
ra từ `future_to_try`. Nếu `timer` hoàn thành trước, `select` sẽ trả về `Right` 
với đầu ra của timer là `()`.
...
hai là với các hàm như `select` và macro như `join!` để kiểm soát cách các future 
bên ngoài được thực thi.
