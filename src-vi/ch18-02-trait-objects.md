...
có thể muốn tạo. Nhưng chúng ta biết rằng `gui` cần theo dõi nhiều giá trị của
các kiểu khác nhau và cần gọi phương thức `draw` trên mỗi giá trị có kiểu khác
nhau này. Nó không cần biết chính xác điều gì sẽ xảy ra khi chúng ta gọi phương
thức `draw`, chỉ cần biết rằng giá trị đó sẽ có phương thức sẵn sàng để gọi.
...
_giống_ hơn với đối tượng trong các ngôn ngữ khác ở chỗ chúng kết hợp dữ liệu và
...
điều phối động, trình biên dịch tạo ra mã mà tại thời điểm chạy sẽ tìm ra phương
thức nào để gọi.
...
ưu hóa, và Rust có một số quy tắc, được gọi là _khả năng tương thích dyn_, về
nơi bạn có thể và không thể sử dụng điều phối động. Những quy tắc đó nằm ngoài
phạm vi của cuộc thảo luận này, nhưng bạn có thể đọc thêm về chúng [trong tài
liệu tham khảo][dyn-compatibility]. Tuy nhiên, chúng ta đã có được sự linh hoạt
bổ sung trong mã mà chúng ta đã viết trong Listing 18-5 và có thể hỗ trợ trong
Listing 18-9, vì vậy đó là một sự đánh đổi cần cân nhắc.
