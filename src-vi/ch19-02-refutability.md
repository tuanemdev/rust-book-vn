...
ta có thể sử dụng `if let`. Sau đó nếu mẫu không khớp, mã sẽ bỏ qua phần mã
...
Chúng ta đã cho mã một lối thoát! Mã này bây giờ hoàn toàn hợp lệ. Tuy nhiên,
nếu chúng ta cung cấp cho `if let` một mẫu không bác bỏ được (một mẫu sẽ luôn
khớp), chẳng hạn như `x`, như được hiển thị trong Listing 19-10, trình biên dịch
sẽ đưa ra cảnh báo.

<Listing number="19-10" caption="Cố gắng sử dụng một mẫu không bác bỏ được với `if let`">
...
Rust phàn nàn rằng việc sử dụng `if let` với một mẫu không bác bỏ được là không
hợp lý:
