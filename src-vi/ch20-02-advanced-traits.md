...
sử dụng kiểu liên kết, chúng ta chỉ có thể chọn kiểu của `Item` sẽ là gì một
lần, vì chỉ có thể có một `impl Iterator for Counter`. Chúng ta không phải chỉ
định rằng chúng ta muốn một iterator của các giá trị `u32` ở mọi nơi mà chúng ta
gọi `next` trên `Counter`.
...
một nơi trú ẩn động vật muốn đặt tên cho tất cả chó con là _Spot_. Chúng ta tạo
