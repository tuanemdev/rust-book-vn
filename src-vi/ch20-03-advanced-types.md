...
Trait Bên Ngoài trên Kiểu Bên Ngoài."][using-the-newtype-pattern]<!-- ignore -->
...
`Millimeters` và `Meters` bọc các giá trị `u32` trong một newtype. Nếu chúng ta
viết một hàm với tham số có kiểu `Millimeters`, chúng ta sẽ không thể biên dịch
một chương trình mà vô tình cố gắng gọi hàm đó với một giá trị có kiểu `Meters`
hoặc một `u32` đơn giản.
