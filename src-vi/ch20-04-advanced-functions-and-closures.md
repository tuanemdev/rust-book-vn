...
Hoặc chúng ta có thể đặt tên một hàm làm đối số cho map thay vì closure. Listing
20-30 cho thấy điều này sẽ trông như thế nào.

<Listing number="20-30" caption="Sử dụng phương thức `String::to_string` để chuyển đổi số thành chuỗi">
...
`FnMut`. Ví dụ, mã trong Listing 20-32 sẽ hoạt động tốt.
...
<Listing file-name="src/main.rs" number="20-33" caption="Tạo một `Vec<T>` của các closure được định nghĩa bởi các hàm trả về `impl Fn`">
...
hàm này đều trả về closure triển khai cùng một trait, `Fn(i32) -> i32`, các kiểu
...
Futures"][any-number-of-futures] trong Chương 17. Chúng ta đã thấy một giải pháp
