...
`proc_macro` đi kèm với Rust, vì vậy chúng ta không cần thêm nó vào phụ thuộc
trong _Cargo.toml_. Crate `proc_macro` là API của trình biên dịch cho phép chúng
ta đọc và thao tác mã Rust từ mã của chúng ta.
...
kiểu được chú thích bằng cách sử dụng `ast.ident`. Cấu trúc trong Listing 20-33
cho thấy rằng khi chúng ta chạy hàm `impl_hello_macro` trên mã trong Listing
20-31, `ident` mà chúng ta nhận được sẽ có trường `ident` với giá trị là
`"Pancakes"`. Do đó, biến `name` trong Listing 20-34 sẽ chứa một thể hiện của
