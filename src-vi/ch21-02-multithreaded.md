...
duyệt: một cho _http://127.0.0.1:7878/_ và cái kia cho
...
tiến, và việc so sánh hai giải pháp sẽ dễ dàng hơn. Listing 21-11 hiển thị các
thay đổi cần thực hiện đối với `main` để tạo một luồng mới để xử lý mỗi luồng
trong vòng lặp `for`.
...
Hãy xem xét: điều gì sẽ khác ở đây nếu chúng ta sẽ thực thi một _future_ thay vì
một closure?
...
pooling. `Worker` nhận mã cần được chạy và chạy mã đó trong luồng của Worker.
...
Sau khi học về vòng lặp `while let` trong Chương 17 và 18, bạn có thể đang tự
hỏi tại sao chúng ta không viết mã luồng công nhân như được hiển thị trong
Listing 21-21.
